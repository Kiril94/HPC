...
SolarSystem random_system(uint64_t num_of_asteroids, uint64_t seed) {
    ...
    sun_and_planets.add(mass, pos_x, pos_y, pos_z, vel_x, vel_y, vel_z);
    ...
    for (uint64_t i = 0; i < num_of_asteroids; ++i) {
        ...
        asteroids.add(mass, pos_x, pos_y, pos_z, vel_x, vel_y, vel_z);
    }
    solar_system = {sun_and_planets, asteroids};
    return solar_system;
}
...
void update_velocity(Bodies &a, const Bodies &b, const int &i, const int &j, const double &dt) {
    ...
    double r = std::sqrt((b.pos_x.at(j) - a.pos_x.at(i)) * (b.pos_x.at(j) - a.pos_x.at(i))
                       + (b.pos_y.at(j) - a.pos_y.at(i)) * (b.pos_y.at(j) - a.pos_y.at(i))
                       + (b.pos_z.at(j) - a.pos_z.at(i)) * (b.pos_z.at(j) - a.pos_z.at(i)));

    // Acceleration:  A = ((G m_b)/r^2)*((x_b-x_a)/r)
    double A = (G * b.mass.at(j) / (r*r)); // Acceleration without direction

    // Update velocity of `a`
    a.vel_x.at(i) += A * ((b.pos_x.at(j) - a.pos_x.at(i)) / r) * dt;
    a.vel_y.at(i) += A * ((b.pos_y.at(j) - a.pos_y.at(i)) / r) * dt;
    a.vel_z.at(i) += A * ((b.pos_z.at(j) - a.pos_z.at(i)) / r) * dt;
}
...
void kick_same(Bodies &a, const double &dt) {
    ...
    uint64_t n = a.pos_x.size();
#pragma acc kernels
    for (uint64_t i = 0; i < n; i++) {
        for (uint64_t j = 0; j < n; j++) {
            if (i != j) {
                update_velocity(a, a, i, j, dt);
            }
        }
    }
}
...
void kick_other(Bodies &a, const Bodies &b, const double &dt) {
    ...
    uint64_t n = a.pos_x.size();
    uint64_t m = b.pos_x.size();
#pragma acc kernels
    for (uint64_t i = 0; i < n; ++i) {
        for (uint64_t j = 0; j < m; ++j) {
            update_velocity(a, b, i, j, dt);
        }
    }
}
...
void drift(Bodies &bodies, const double &dt) {
    ...
    int n = bodies.pos_x.size();
    for (int i = 0; i<n; i++) {
        bodies.pos_x.at(i) += bodies.vel_x.at(i) * dt;
        bodies.pos_y.at(i) += bodies.vel_y.at(i) * dt;
        bodies.pos_z.at(i) += bodies.vel_z.at(i) * dt;
    }
}
